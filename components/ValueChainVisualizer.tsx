  ...
  return (
    <div className="relative w-full h-full p-8 lg:p-16">
      <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
        ...
        {actors.slice(0, -1).map((actor, i) => {
          ...
          return (
            <path
              key={`path-${actor.id}`}
              d={`M ${actor.x} ${actor.y} C ${actor.x + (next.x - actor.x)/2} ${actor.y}, ${actor.x + (next.x - actor.x)/2} ${next.y}, ${next.x} ${next.y}`}
              fill="none"
      ...
      {actors.map((actor) => {
        ...
        const statusColor = iot?.status === 'warning' ? 'bg-amber-400' : 'bg-emerald-500';
        ...
        return (
          ...
          >
            ...
            `}>
              <div className="bg-slate-900/90 backdrop-blur text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg uppercase tracking-wider">
                {actor.name}
              </div>
            </div>
          </button>
